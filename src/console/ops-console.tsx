    ...
    <div className={cx(styles.prototype, styles.console)}>
      <header className={styles.consoleHeader}>
        ...
        <nav aria-label='Primary'>
          <Link href={props.href('dashboard')} aria-current={props.screen === 'dashboard' ? 'page' : undefined}>Overview</Link>
          <Link href={props.href('orders', 'orders')} aria-current={props.screen === 'orders' || props.screen === 'order' || props.screen === 'project' ? 'page' : undefined}>Work ledger</Link>
          <Link href={props.href('orders', 'projects')} aria-current={props.screen === 'orders' && props.queueMode === 'projects' ? 'page' : undefined} style={props.screen === 'orders' && props.queueMode === 'projects' ? { borderBottomColor: 'var(--accent)', color: 'var(--ink)' } : undefined}>Projects</Link>
          <Link href={props.href('orders', 'kanban')} aria-current={props.screen === 'orders' && props.queueMode === 'kanban' ? 'page' : undefined} style={props.screen === 'orders' && props.queueMode === 'kanban' ? { borderBottomColor: 'var(--accent)', color: 'var(--ink)' } : undefined}>Kanban</Link>
        </nav>
