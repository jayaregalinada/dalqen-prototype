      ...
      <div className={styles.tableShell}>
        <table>
          ...
          <tbody>{props.projectRows.map((row) => (
            <tr key={row.order + '-' + row.project} onClick={() => window.history.pushState(null, '', props.href('project'))} style={{ cursor: 'pointer' }}>
              ...
              <td><Link href={props.href('project')} aria-label={'Open ' + row.project} onClick={(e) => e.stopPropagation()}><ArrowRight size={16} /></Link></td>
            </tr>
          ))}</tbody>
        </table>
      </div>
    ...
    <div className={styles.tableShell}>
      <table>
        ...
        <tbody>{props.orderRows.map((row) => (
          <tr key={row.ref} onClick={() => window.history.pushState(null, '', props.href('order'))} style={{ cursor: 'pointer' }}>
            ...
            <td><Link href={props.href('order')} aria-label={'Open ' + row.ref} onClick={(e) => e.stopPropagation()}><ArrowRight size={16} /></Link></td>
          </tr>
        ))}</tbody>
      </table>
    </div>
