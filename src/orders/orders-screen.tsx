...
export function OrdersScreen({ props, flavor }: { props: PrototypeProps; flavor: string }) {
  return (
    <div className={styles.standardScreen}>
      <ScreenTitle eyebrow="Production ledger" title={props.queueMode === "orders" ? "Orders" : 'Project queues'}
        copy={props.queueMode === 'orders' ? 'Track every job order from intake to final release.' : 'Move work by stage, owner, and department.'}
      ...
      <div className={styles.listControls}><QueueToggle mode={props.queueMode} setMode={props.setQueueMode} /><div className={styles.filterSet}>
        <button type="button"><Filter size={15} /> Saved: {props.queueMode === "orders" ? "Active orders" : 'My department'}<ChevronDown size={14} /></button>
